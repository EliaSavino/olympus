...
@pytest.mark.parametrize(
    "name, low, high, stride",
    [("temperature", 50.0, 100.0, 10.0), ("pressure", -1.0, 1.0, 0.2)],
)
def test_discrete_parameters(name, low, high, stride):
    param = ParameterDiscrete(name=name, low=low, high=high, stride=stride)
    for attr, value in zip(
        ["name", "low", "high", "stride"], [name, low, high, stride]
    ):
        assert param.get(attr) == value


@pytest.mark.parametrize(
    "name, options, descriptors",
    [
        (
            "organic_cation",
            ["methylammonium", "ethylammonium"],
            [[2.0, 32.065], [3.0, 46.09]],
        ),
...
def test_mixed_param_space():
    ...
    cont_args = {"name": "param_0", "low": 0.0, "high": 1.0}
    param_space.add(
        ParameterContinuous(
            name=cont_args["name"],
            low=cont_args["low"],
    ...
    disc_args = {"name": "param_1", "low": 0.0, "high": 1.0, "stride": 0.1}
    param_space.add(
        ParameterDiscrete(
            name=disc_args["name"],
            low=disc_args["low"],
