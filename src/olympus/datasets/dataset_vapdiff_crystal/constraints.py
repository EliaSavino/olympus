...
max_mol_constr = pickle.load(
    open(
        f"{__home__}/datasets/dataset_vapdiff_crystal/max_molarity_constraints.pkl",
        "rb",
    )
)
...
df_hull = pd.read_csv(
    f"{__home__}/datasets/dataset_vapdiff_crystal/convex_hull_chemnames.csv"
)
...
def known_constraints(param_vec):
    ...
    # check convex hull
    target_molarities = {
        organic: organic_molarity,
        "Lead Diiodide": inorganic_molarity,
        solvent: solvent_molarity,
...
def backward_check(target_molarities, alpha_vial_volume, df_hull):
    ...
    with gp.Env(empty=True) as env:
        ...
        with gp.Model(env=env) as m:
            ...
            v = []
            for i in range(n_ss):
                v_i = m.addVar(lb=0.0, name=str(i))  # add var
                v.append(v_i)
                m.addConstr(v_i >= 0.0, name=str(i))
            ...
            for j in range(n_chem):
                ...
                m.addConstr(
                    c == alpha_vial_volume * target_molarities_padded[j],
                    name=chemnames[j],
                )
