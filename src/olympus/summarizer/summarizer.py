...
class Summarizer:
    def __init__(self, campaigns=[], include_stats=True):
        ...
        if self.include_stats:
            ...
            self.stats_map = {
                "mean": self.analyzer.get_best_mean,
    ...
    def _load_json(self):
        ...
        self.dataset_bib = json.load(
            open("olympus/summarizer/dataset_bib.json", "r"), strict=False
        )
        self.planner_bib = json.load(
            open("olympus/summarizer/planner_bib.json", "r"), strict=False
        )
        ...
        for planner_name, campaigns in sorted_campaigns.items():
            ...
            for stat in stats_list:
                if stat == "mean":
                    ...
                    res = self.stats_map[stat](campaigns=campaigns, locs=locs)
                if len(res) == 1:
                    res = res[0]
    ...
    def to_latex(self, filename="summary.tex", stats_list=["mean"]):
        ...
        """
        self._load_json()
        caption = self._write_table_capton(
            self.campaigns[0]["dataset_kind"],
        ...
        if self.include_stats:
            ...
            latex_tab = df.to_latex(
                index=True, caption=caption, label="tab:benchmark_results"
            )
        else:
