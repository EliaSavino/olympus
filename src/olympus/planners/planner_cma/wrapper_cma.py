...
class Cma(AbstractPlanner):
    ...
    def _set_param_space(self, param_space):
        ...
        if self.init_guess is None:
            ...
            self.init_guess = np.random.uniform(
                low=0,
                high=1,
                size=(
                    len(
    ...
    def _priv_evaluator(self, params):
        params = (
            self.param_space.param_uppers - self.param_space.param_lowers
        ) * params + self.param_space.param_lowers
    ...
    def create_optimizer(self):
        ...
        opts = {
            "bounds": [
                list(np.full((len(self.param_space)), 0)),
