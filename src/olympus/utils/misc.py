    ...
    if n == 1:
        ...
        for i in itertools.product(range(2), repeat=l):
            ...
            if np.size(new_pareto_front) > 0:
                child_lower, child_upper = get_cells(
                    new_pareto_front, new_ref, new_ref_inv
                )
...
def get_r2(objs, w_ref, num_weights=100):
    """Compute the r2 indicator. This assumes that all objectives have a goal
    "minimize". The user must flip the signs of "maximize" objectives before
    calling this function.


    Args:
            objs (np.ndarray): objective values
            w_ref (np.ndarray): reference point for r2 indicator
    """
    W = np.random.sample((num_weights, objs.shape[1]))
    sum_W = np.sum(W, axis=1).reshape(-1, 1)
    W = W / sum_W

    sum_terms = []
    for w in W:
        t = w * np.abs(objs - w_ref)
        t_max = np.amax(t, axis=0)
        min_t_max = np.amin(t_max, axis=0)
        sum_terms.append(min_t_max)
    r2 = (1.0 / num_weights) * np.sum(sum_terms)
    return r2


if __name__ == "__main__":

    objs_1 = np.array([[0.3, 0.1], [0.9, 0.7], [0.5, 0.5]])
    objs_2 = np.array([[0.2, 0.05], [0.4, 0.3], [0.1, 0.0]])
    w_ref = np.array([0.0, 0.0])
    num_weights = 100

    r2_1 = get_r2(objs_1, w_ref, num_weights)
    r2_2 = get_r2(objs_2, w_ref, num_weights)
